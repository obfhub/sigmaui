...
#pragma mark - Scene-safe key window (iOS 13+ / iOS 18 safe)
...
static UISwitch *AddToggle(UIView *panel, NSString *name, CGFloat y, BOOL initial, SEL action) {
    ...
    [sw addTarget:gHandlers action:action forControlEvents:UIControlEventValueChanged];
    [panel addSubview:sw];
...
static void BuildUI(UIWindow *window) {
    ...
    if (icon) {
        ...
        [gLogo setTitleColor:[UIColor colorWithRed:0.8 green:0.1 blue:0.9 alpha:1.0]
    ...
    gLogo.layer.shadowColor = UIColor.blackColor.CGColor;
    gLogo.layer.shadowOpacity = 0.35;
    gLogo.layer.shadowRadius = 8;
    gLogo.layer.shadowOffset = CGSizeMake(0, 4);

    [gLogo addTarget:gHandlers action:@selector(togglePanel)
    forControlEvents:UIControlEventTouchUpInside];

    UIPanGestureRecognizer *pan =
        [[UIPanGestureRecognizer alloc] initWithTarget:gHandlers action:@selector(handleDrag:)];
    ...
    AddToggle(gPanel, @"God Mode", 58, gGodMode, @selector(toggleGod:));
    AddToggle(gPanel, @"Wallhack", 110, gWallhack, @selector(toggleWall:));
    AddToggle(gPanel, @"Aimbot", 162, gAimbot, @selector(toggleAim:));
    ...
    slider.value = gSpeed;
    [slider addTarget:gHandlers action:@selector(speedChanged:)
...
#pragma mark - Entry (runs on dylib load)
